...
data = pd.read_csv("./data/3.housing.csv", names=header, delim_whitespace=True)
print(data)
# 데이터 전처리 : Min-Max 전처리
array = data.values
X = array[:, 0:13]
...
plt.scatter(range(len(Y_pred[:15])), Y_pred[:15], color='red', label='Predicted Values', marker='x')
